...
// call prefix  T1 is T1xxx  Note that no non standard telemetry transmission is implemented in the this code, but
// there is example code.
// These provide slightly less space for data encoding because it must be of the format e.g. T1Nabc.  "N" must be a number
// and "abc" can only be letters. Custom telemetry messages can chose prefixes that are not assigned to any country.
